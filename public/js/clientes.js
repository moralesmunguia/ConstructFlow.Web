/**
 * clientes.js v3.1 - Corregido: carga vía axios para respetar el token JWT global
 */
...
document.addEventListener('DOMContentLoaded', () => {
    initDataTable();
    cargarClientes();          // <-- carga inicial con axios
    cargarEmpresas();
...
function mostrarListado() {
    ...
    cargarClientes();          // <-- recarga con axios, no ajax.reload
}
...
/* ---- NUEVA: carga con axios (respeta el token JWT global) ---- */
        ...
        if (r.isConfirmed) {
            axios.delete(`${API_CLIENTES}/${id}`)
                .then(() => {
                    ...
                    cargarClientes();   // <-- recarga con axios
                })
                .catch(err => {

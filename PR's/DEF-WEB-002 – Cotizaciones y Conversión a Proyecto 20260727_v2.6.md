...
# 29. Matriz funcional
  ...
  COT-039   Duplicar                       RESUELTO (2026-07-27) -- `CotizacionService::duplicar()` copia encabezado (folio nuevo, Fecha=hoy, vigencia relativa conservada, fuerza EstadoCotizacionID=BORRADOR), partidas y actividades (ActividadPadreID remapeado en dos pasadas). Nuevo `POST /api/v1/cotizaciones/{id}/duplicar` + botón "Duplicar" en el grid Web. De paso se corrigió un bug preexistente critico: `CotizacionRepository::insertHistorialEstado()` nunca leia `$data` (bind con variables indefinidas) -- rompia el historial inicial de TODA cotizacion nueva, no solo la duplicada
  COT-040   Folio corto/interno            TERMINADO
  COT-041   Trazabilidad CotizacionActividadID -> ActividadID   RESUELTO (2026-07-27) -- se agregó la columna real `CF_Actividad.CotizacionActividadID` (BIGINT NULL, con índice y FK hacia `CF_CotizacionActividad`, aplicada en BD). `ProyectoRepository::insertActividad()` (usado por la conversión) y `ActividadRepository::insertActividadProyecto()` (usado por el alta rápida Web) ya la soportan -- NULL para actividades que no vienen de una conversión. Requirió `DEF-WEB-004_cot041_trazabilidad_actividad.sql` (ajustado a BIGINT para calzar con el tipo real de la PK referenciada)
...
# 33. Historial
  ...
  2.5 (2026-07-27)                    Implementado COT-039 (Duplicar
                                      cotización): nuevo
                                      `CotizacionService::duplicar()`,
                                      endpoint `POST
                                      /api/v1/cotizaciones/{id}/
                                      duplicar` y botón en el grid
                                      Web. Se corrigió de paso un bug
                                      preexistente crítico en
                                      `CotizacionRepository::
                                      insertHistorialEstado()` (nunca
                                      leía `$data`, bind con
                                      variables indefinidas) que
                                      rompía el historial inicial de
                                      toda cotización nueva.

  2.6 (2026-07-27)                    Implementado COT-041
                                      (trazabilidad
                                      CotizacionActividadID ->
                                      ActividadID): nueva columna
                                      `CF_Actividad.
                                      CotizacionActividadID` (BIGINT
                                      NULL + índice + FK, aplicada en
                                      BD real), soportada en ambos
                                      caminos de inserción de
                                      actividades de Proyecto
                                      (conversión y alta rápida
                                      Web).
